...
export function ActionMultipleDecisionView({
  ...
  useEffect(() => {
    const deltas = selectedActions.reduce(
      (sum, action) => ({
        career: sum.career + action.parameter_change.career,
        relations: sum.relations + action.parameter_change.relations,
        health: sum.health + action.parameter_change.health,
        money: sum.money + action.parameter_change.money,
      }),
      { career: 0, relations: 0, health: 0, money: 0 },
    );
    gameStore.tempSetParameterModifications(deltas);
  }, [selectedActions, gameStore.tempSetParameterModifications]);
